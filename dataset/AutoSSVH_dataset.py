...
class TrainDataset(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        visual_word = np.array(t1)
        mask = self.maskgenerator()

        output = {"mask": torch.as_tensor(mask),
                    "visual_word": torch.as_tensor(visual_word),"index":torch.as_tensor(item)}
...
def get_AutoSSVH_train_loader(cfg, shuffle=True, num_workers=1, pin_memory=True):
...
def get_AutoSSVH_eval_loader(cfg, shuffle=False, num_workers=1, pin_memory=False):
